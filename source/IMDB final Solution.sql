...
-- Type your code below:
select year, country,count(id) as no_of_movies
from movie
...
rank() over(order by production_company desc) as prod_company_rank
from movie as m
inner join ratings as r
on m.id = r.movie_id
...
inner join movie as m
on g.movie_id = m.id
where m.country = 'USA' and month(date_published)=3 and r.total_votes>1000 and year=2017
group by genre
order by movie_count desc;
